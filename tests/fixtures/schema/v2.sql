-- Frozen snapshot of library schema version 2 (v1 plus track.missing_since, E3-S12) with one row in every
-- table. The track DDL keeps the shape ALTER TABLE ADD COLUMN leaves in sqlite_master (the new column appended
-- after the original text's last newline, no whitespace before the closing paren), which is what the migrator test compares. Loaded by LibraryMigratorTests: the runner must bring it to the latest version and end with exactly the
...
INSERT INTO schema_version(version, applied_at) VALUES (2, 1757376000000);
