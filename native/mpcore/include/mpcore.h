 ...
 * 0.16 adaptive quality (E4-S7): mp_renderer_set_quality is implemented over a controller on the render
 * thread, and mp_render_stats grows a tail saying what that controller decided and what it decided it on.
 * Same reading as 0.8 to 0.15 for the export - declared and stubbed since 0.3, beginning to work, which is new
 * function and so a minor - and 0.12's reading for the struct: the seven fields are APPENDED, nothing before
 * them moved, so a caller built against any earlier minor passes the struct_size it always passed and is
 * served exactly the prefix it understands. That is the thing T-140 bought and this is the first story to
 * spend it. One new enum, mp_render_cost_source, names which of two measurements the controller is acting on;
...
/* Which measurement the quality controller is deciding on (ABI 0.16). The frame-to-frame interval is not a
...
typedef struct mp_render_stats {
    ...
    uint8_t visible;
    char adapter[128];
    /* ---- adaptive quality (ABI 0.16, appended) ----
