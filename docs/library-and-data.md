...
4. **ExtractArt** is `IArtCache.StoreAsync(embeddedPicture, audioPath)` behind a two-worker gate: it takes the embedded picture the reader already found (front cover preferred), else the first of `cover.*`, `folder.*`, `front.*`, `*.jpg` in the file's folder, hashes the bytes, skips when the hash directory exists, otherwise decodes with `Windows.Graphics.Imaging` and writes the three sizes plus `palette.json` (median-cut over 300 px image, five colours, sorted by population, with relative luminance), and returns the track and album art hashes to record. E3-S7 implements it; without one the stage passes the track through.
