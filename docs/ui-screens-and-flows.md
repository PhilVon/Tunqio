...
| | *As decided (Q-72): one preset for every mode, `viz.preset`, with Ambient Glow as the first-run default; a mode does not choose or remember a preset of its own.* | | |
...
As built (E5-S1), the mode is `ShellState`, a container singleton that reads and writes `ui.mode` and has no route to `PlaybackSession` at all, which is how "never interrupts playback" is kept rather than hoped for. It remembers the mode Focus was entered from, so Esc and F11 go back there (flow 7); a launch that opens in Focus goes back to Discovery. The widths in the table above compose with the window-size table rather than replacing it: `ShellLayout.For(width, mode)` gives Discovery the E2-S1 shares unchanged, gives Focus the whole width at every size with the sidebar collapsed (collapsed, not removed, so the library pane keeps its page and back stack), and gives Curation the two shares swapped wherever the panels are columns — 40 / 60 from 1200 px, 1 : 2 between 800 and 1200, where Now Playing's 400 px floor wins — and the equal heights when they stack (Phil, Q-67). The switcher is a `RadioButtons` in the controls panel, so Narrator reads the selected mode without a name having to carry it. The transition is a 250 ms opacity fade over the new shape on the panels' composition visuals; the grid resizes in one layout pass and is not itself animated.

Focus's own behaviour (E5-S2) is `FocusChrome`, a pure class over `ShellState` and a `TimeProvider`. In Focus the controls fade out after 3 s with no pointer movement and no key, and in the other two modes they never hide. Coming back is not timed: any pointer move (listened for with `handledEventsToo`, since a list under the pointer handles its own moves) or any key shows them synchronously, inside the handler that saw the input, which is how the 100 ms in E5-S2's criterion is met. Hiding is opacity rather than visibility, so the transport stays in the tree and focusable, as the accessibility contract asks. Three pins hold the controls on screen whatever the clock says: the pointer over the bar, keyboard focus inside it, and the queue flyout open. Only *keyboard* focus pins (`FocusChrome.PinsOnFocus`): a click on the mode switcher also lands focus in the bar, and the first live run of `tools/check-focus.ps1` showed that pinning on any focus stopped the controls from ever hiding in a Focus entered by the switcher. A hide is logged with the idle time the app itself measured ("hidden after 3 s without input"), because Windows delivers input of its own about a second after the pointer stops, and a check timing from its own last move would otherwise report a correct hide as late. The left edge is an 8 px strip that opens a queue peek, a second `QueuePanel` over the same view model, closed when the pointer leaves it. Double-clicking the art enters Focus. Track changes in Focus are spoken through a UIA notification (`RaiseNotificationEvent`, processing `CurrentThenMostRecent`, so Narrator finishes what it is saying first) raised on the Now Playing panel once per real track change. The first cut raised `LiveRegionChanged` on an invisible 1 × 1 text block, and Narrator never spoke it (Q-71). `TrackAnnouncer` survives only to hold the text for `tools/check-focus.ps1` to read. Touch swipe from the edge is not built; the strip answers a pointer.

**Hover preview mechanics (Discovery).** Preview uses a separate decode stream added to the mixer at −12 dB with a 200 ms ramp, starting at 30% into the track (or 0 if under 60 s). It is bypassed for the analysis tap so the visualizer keeps following the main track. Only one preview at a time; moving between tiles queues the next after the current fade-out. As built (E5-S5, ABI 0.18): the analysis tap is a DSP on the main mixer, so the preview is not a channel on that mixer but a second decode mixer of its own, read in the pull stage after the main stage; that is what keeps it out of the tap. It plays the file on a stream of its own (so the playing track can be previewed), the main mix ducks by 6 dB over the same 200 ms ramp, the volume slider applies to it but the pause envelope does not, and it fades itself out after 15 s. `mp_preview_start` refuses a second preview while the first is still audible, fade included; queueing the next tile is the managed side's job.
