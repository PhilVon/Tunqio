...
**Where analysis becomes preset input, in order (as of ABI 0.19).** Each frame the render thread (1) takes whatever the analysis has published into its history ring (`poll_analysis`), (2) chooses which of those frames the picture is drawn from — the one the listener is hearing, E4-S8's `mp_renderer_set_av_sync` — and (3), when temporal smoothing is on, eases that chosen frame through the attack and decay envelope (T-184) before (4) `b0`, `Spectrum` and `Waveform` are written from it. The latency probe describes step 2, the frame *chosen*; the envelope is after it and is not in `mp_latency_sample`.

**Temporal smoothing (T-184, ABI 0.19)** is `mp_renderer_set_temporal_smoothing(renderer, attack_ms, decay_ms)`, one renderer-wide setting that outlives a preset switch, in `native/mpcore/src/render/temporal_envelope.h`. Per value, per frame, `y = x + (y − x)·exp(−dt/τ)` with τ the attack while the input is above the value and the decay otherwise, and `dt` the render thread's real frame interval: exponential in *time*, so for an input held between frames the curve at 144 Hz passes through the same values at the same instants as at 60 Hz (asserted to 2e-5 on the envelope and to one byte on a captured picture). It eases the magnitudes — `spectrum`, `bands`, `level.x` (rms), `level.y` (peak) — and deliberately not `Waveform` (signed samples of a different stretch of audio each hop, which an envelope would only flatten), `level.z`/`level.w` (centroid and harmonic ratio, positions on a scale where T-181 measured the *middle* painting the brightest field) or `counts.x` (an onset is an event). A value within 1e-7 of its input takes the input, so a decay to silence arrives at exactly zero and never passes through subnormal floats; a NaN never outlives its frame; a moved `discontinuities` count restarts the envelope at its input. **Off is both time constants at zero, the default, and off does not run the envelope at all**: the frame chosen is uploaded as it is, which is why every golden image is unchanged and why a renderer switched off after easing draws the untouched picture byte for byte. While it is on the spectrum is re-uploaded on every frame the envelope moves; the waveform still only when the sequence has. The cost is responsiveness: a rise reaches half height after `attack·ln 2`, in whole frames, on top of the av-sync latency — 0 ms at the fastest (rise 0) and at the default 20 ms rise at 60 Hz, one refresh (6.9 ms) for the default at 144 Hz, 167 ms at the slowest rise the page offers (250 ms). The flash search runs again with the envelope at its fastest attack and at its slowest decay, every preset, colour mode and centroid, nine frames each against the silence before them and the frame before them; the worst figures are the same as with it off (0.0376 mean and 10.83% area for Spectrum Bars), because every eased value lies between two values the analysis really published.

**Which preset is drawing (T-127, ABI 0.21)** is `mp_renderer_get_preset(renderer, mp_preset_info* out)`: the active preset's catalogue entry, under the same `struct_size` rule as `mp_renderer_get_stats`. It is what the picture is, not what was last asked for: a refused `mp_renderer_set_preset` leaves it, and so does a rescan that no longer lists it. `VisualizationHost.ActivePresetId` is read back from it after attaching, switching, changing the user root and rescanning, where before it remembered the last request plus the fact that the core starts on its first entry.
...
**`smoothing` is spatial, not temporal, and that is a limitation of the contract rather than a choice.** In Spectrum Bars and Radial Spectrum it widens a bar's or a ray's slice into its neighbours and slides the reading from that slice's peak toward its mean; in Waveform it is the width of a box filter along the hop; in Ambient Glow it slides a band group's reading from its peak toward its mean. What a visualizer usually means by smoothing — an attack/decay envelope — needs state that survives between frames, and a preset has none: `b0` and the two SRVs are the whole surface and there are no UAVs. Temporal smoothing has to be the renderer's, applied to the spectrum before it is uploaded, which makes it a property of the renderer rather than of one preset — and since T-184 it is (see "Temporal smoothing" above). Because the word promises the envelope, the parameter keeps the name `smoothing` but is labelled for what it does (T-141, Q-90): **Spread** in Spectrum Bars and Radial Spectrum, **Softness** in Waveform, **Peak to average** in Ambient Glow (which already has a Spread). Temporal smoothing in the renderer is T-184.
