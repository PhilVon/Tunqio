...
- **Signing (Q-5, superseded by D-34):** releases stay self-signed for the foreseeable future; Phil chose on 2026-09-14 not to pay for a certificate or a signing subscription, so no real identity is planned. The manifest publisher stays `CN=Tunqio`, the subject of the self-signed certificate, and is frozen at 1.0. Every release is signed by a certificate with that subject, and anyone installing trusts it once per machine, as the README describes. Where releases are built and signed (GitHub Actions on a version tag, or a local script) and where the key is held are open on T-80 (Q-128).
