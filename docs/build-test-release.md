...
| mpcore/render | Golden image | Catch2 on WARP, readback compare | Each built-in preset renders a fixed frame to within tolerance of a checked-in PNG; every preset compiles; luminance-flash test (< 3 Hz full-field change). E4-S3 built the readback half of this (`renderer::capture_pixel`) and uses it on solid-colour preset fixtures under `native/mpcore.tests/fixtures/presets`, which is what a *loader* needs to prove: which preset is on the target, and that a preset that will not compile does not replace it. The checked-in PNGs belong to E4-S4/E4-S5, with the presets they are of |
...
4. `msbuild Tunqio.sln -p:Configuration=Release -p:Platform=x64 -warnaserror` (also builds `mpcore.tests`), then `tools/check-presets.ps1` over the built app.
