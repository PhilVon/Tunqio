...
4. Gapless: `BASS_Mixer_ChannelSetSync(current, BASS_SYNC_END | BASS_SYNC_MIXTIME, ...)` fires at mix time and the pre-opened next stream is added there with `BASS_Mixer_StreamAddChannelEx`. E1-S2 measured that a source added inside the sync starts at the *buffer* boundary, not the sync frame, so every source is added with `BASS_MIXER_CHAN_LIMIT`: the mixer read stops on the ending source's last frame, the successor is the next frame read, and the pull stage loops to fill the rest of the buffer. Eleven of twelve fixture formats join at the exact frame, including 44.1 kHz sources resampled into the 48 kHz mixer; AAC needed the engine to trim the priming and padding Media Foundation delivers, read from the MP4's edit list or `iTunSMPB` and applied by a user decode stream over the MF stream (T-102); WMA through Media Foundation is best-effort ([spikes/e1-s2-gapless-join.md](spikes/e1-s2-gapless-join.md)).
