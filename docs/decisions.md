...
2. Streams are added to one mixer (`BASS_Mixer_StreamCreate(48000, 2, BASS_SAMPLE_FLOAT | BASS_STREAM_DECODE | BASS_MIXER_NONSTOP)`) with `BASS_MIXER_CHAN_NORAMPIN` and `BASS_MIXER_CHAN_BUFFER`. Mixer rate follows the output device rate. E1-S6 measured that this has to hold in shared mode too, not only exclusive: BASSWASAPI honours a rate that differs from the device's in shared mode, so a mixer left at 48 kHz drove a 44.1 kHz device and Windows resampled every frame on the way out. The device is therefore opened at its own mix format (`BASS_WASAPI_DEVICEINFO.mixfreq`/`mixchans`) in both modes and the mixer is rebuilt at whatever `BASS_WASAPI_GetInfo` says was granted, carrying the playing source over at its position.
3. Output is a basswasapi device whose `WASAPIPROC` pulls from the mixer with `BASS_ChannelGetData`. BASS is initialised with the "no sound" device. Exclusive mode (R-3) is only ever a request: a driver may refuse it because another application holds the device or because no offered format is accepted, and the engine then opens the same device in shared mode and raises `MP_EVENT_ERROR` naming the device and the BASS error rather than leaving the user without output. `mp_engine_stats.exclusive` is what was granted, so the shell reads the mode back instead of assuming the one it asked for. A shared-mode failure has nothing to fall back to and is returned to the caller with no event (E1-S6). Device changes (E1-S7) come from `BASS_WASAPI_SetNotify` alone — BASSWASAPI's notification callback is an `IMMNotificationClient` already, so the second one the design called for would only deliver the same events twice. That callback runs on a Windows notification thread and must not reopen a device from inside itself, so it enqueues and a watch thread does the work under the control mutex. Losing the open device parks playback where it stood and closes the output (`MP_EVENT_DEVICE_LOST`), and the device coming back is offered rather than taken (`MP_EVENT_DEVICE_CHANGED`, `b = 0`): the shell decides, because ui-screens-and-flows.md forbids continuing silently on a device the user did not choose. The one migration the engine makes for itself is the default moving under a caller that asked for `MP_DEVICE_DEFAULT` (`b = 1`), which is what following the default means.
4. Gapless: `BASS_Mixer_ChannelSetSync(current, BASS_SYNC_END | BASS_SYNC_MIXTIME, ...)` fires at mix time and the pre-opened next stream is added there with `BASS_Mixer_StreamAddChannelEx`. E1-S2 measured that a source added inside the sync starts at the *buffer* boundary, not the sync frame, so every source is added with `BASS_MIXER_CHAN_LIMIT`: the mixer read stops on the ending source's last frame, the successor is the next frame read, and the pull stage loops to fill the rest of the buffer. The loop retries once on a read that returned nothing but delivered a join, because when that last frame is also the pull's first frame the read returns zero: the sync that adds the successor runs inside it, and only the read after it can reach the successor. Without that retry a join landing on a period boundary dropped a buffer and counted an underrun (T-108, found by the E1-S11 soak; the earlier per-format measurements had used 479-frame pulls, which put every seam inside a buffer). Eleven of twelve fixture formats join at the exact frame, including 44.1 kHz sources resampled into the 48 kHz mixer; AAC needed the engine to trim the priming and padding Media Foundation delivers, read from the MP4's edit list or `iTunSMPB` and applied by a user decode stream over the MF stream (T-102); WMA through Media Foundation is best-effort ([spikes/e1-s2-gapless-join.md](spikes/e1-s2-gapless-join.md)).
5. User crossfade (0–12 s) and a 50 ms guard fade on hard stops, seeks and manual skips. The crossfade was to be `BASS_ChannelSlideAttribute(BASS_ATTRIB_VOL)`; E1-S5 measured that a slide on a source inside a mixer never advances, so E1-S4 gives each source its own equal-power envelope in a DSP (`fade_proc`) that the mixer runs as it decodes that source, keyed to the source's own frame position (the outgoing follows cos, the incoming sin, so their powers sum to one), with BASSmix applying the source's `BASS_ATTRIB_VOL` (ReplayGain) on top. A mix-time `BASS_SYNC_POS` sync on the playing source adds the queued one at the fade point the way the END sync adds it at the end; the incoming therefore starts at the beginning of the output buffer being mixed (finding 1 of the E1-S2 spike, up to one buffer early), which a crossfade of seconds does not notice and the gapless join, which keeps LIMIT, does not suffer. The join is chosen per boundary by the caller through `mp_engine_preload_next_ex(MP_JOIN_GAPLESS | MP_JOIN_CROSSFADE)` because only the session knows the albums (`CrossfadePolicy`: same album and gapless on means gapless); with no crossfade set, an unknown length, or a fade point already passed (also by a seek into the window) the join falls back to the gapless one. Stop, seek and a manual play cut a crossfade's outgoing tail under the guard fade. E1-S1 put the guard fade and the volume in the engine's own pull stage rather than in BASS attributes: the audio thread owns a linear envelope that ramps over 50 ms, pause asks for silence and then holds (the output keeps running with silence and the mixer is not advanced, so the position freezes and resume is immediate), stop/seek/manual play fade out first and wait for the ramp on a live device, and volume is an audio taper (`10^(2(v-1))`, -20 dB at half travel) interpolated across each buffer so a mute lands within one buffer without a click.
   ReplayGain (E1-S5) is per source, not per engine: `mp_track_set_replaygain` resolves the caller's gain (tag plus preamp) and tagged peak to one linear gain, reduced to 1/peak when a known peak would clip, and sets it as the source's `BASS_ATTRIB_VOL`, which BASSmix applies as it mixes that source. The gain therefore rides into the mixer with a preloaded track and is in force on the exact frame of the gapless join, survives seeks and output changes, and a change to the playing source is ramped by the mixer. Measured while doing it: `BASS_ChannelSlideAttribute` on a decoding source inside a mixer never advances (the mixer does not process source slides), so the user crossfade in E1-S4 cannot use it as written above and will need the engine's own envelope or a mixer volume envelope (`BASS_Mixer_ChannelSetEnvelope`).
6. Headless output: `mp_output_config.device_index = MP_DEVICE_NONE` opens no device and `mp_engine_render` pulls what the WASAPI thread would have taken, so the native and Interop suites test the whole transport, the clock and the event stream without a sound device. A Debug hook (`mp::rt::scope`, `common/rt_guard.h`) replaces `operator new` and counts allocations made inside the pull stage; the suite asserts zero.
7. Analysis tap: a `BASS_ChannelSetDSP` callback on the mixer copies float frames plus the mixer byte position into a lock-free ring buffer. Built in E1-S8 as `mpcore/analysis/tap.h`. On the *mixer* rather than on the engine's own pull stage, so analysis sees the mix as the mixer makes it, before the guard envelope and the volume taper: the visualization follows the music, not the volume slider. It stages whatever sized pieces BASS reads into fixed 512-frame hops (ADR-010) and publishes each with the byte position of its first frame, in the same units as `mp_clock.mixer_byte_pos`, so a hop lines up with what is being heard exactly as a gapless join does. That position is counted in the tap rather than read back from BASS: the DSP runs inside the mixer's own `BASS_ChannelGetData`, where asking the mixer where it is would be a question about the read in progress. A consumer that falls behind loses the newest hop and is told how many — evicting the oldest would mean the producer moving the ring's head, which belongs to the consumer alone and is the whole basis of the ring being lock-free. Measured at 0.0008 ms per 480-frame callback, allocating nothing. The native analysis thread runs a real FFT (2048-point, Hann window, 512-frame hop) using **pffft** (BSD-style licence, SIMD) on that data. No IPP, no FFTW. The interim "use BASS's built-in FFT" idea from the first draft is dropped now that the core is native.
...
**As built (E4-S8, ABI 0.17, 2026-09-12).** The mechanism is `mp_renderer_set_av_sync` over a short history of the analysis frames the render thread has seen, and the harness is `tools/LatencyRunner` (the name above was written before it existed). Three things came out differently from the sketch, and all three are measured in [spikes/e4-s8-latency-floor.md](spikes/e4-s8-latency-floor.md).

1. **The uncompensated error is on the other side.** The analysis runs at *mix* time and the mixer is ahead of the loudspeaker, so a renderer drawing the newest frame is 59–62 ms **early**, not late. Compensation is a deliberate delay and the output buffer is the budget it is paid out of. Compensated, the error is +0.04 to +3.8 ms median and 11.4–11.9 ms at p95 in magnitude, so the target is met on everything this can see.
2. **The live buffer depth, not the static latency.** `BASS_WASAPI_GetData(BASS_DATA_AVAILABLE)` — what `mp_clock.output_buffered_bytes` already carried — is read per frame instead of `GetInfo`'s nominal figure. It measured 73–76 ms against a `mp_engine_stats.output_buffer_ms` of 40, so the static number would have been wrong by 35 ms. DXGI frame statistics are not consulted at all: they count refreshes, not photons, and are absent on the headless path where the harness measures.
3. **The target cannot be a gate.** The present-to-photon edge is not visible from inside the process, and the floor of what is visible is the render tick rather than anything about the audio or the GPU — an RTX 4080, WARP, and WARP running the most expensive preset all land within 0.4 ms of each other. So p95 is *reported* with its distribution, and what the build gates is the arithmetic of the frame selection.
