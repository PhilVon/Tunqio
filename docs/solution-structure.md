...
- Reserved bytes (T-145, decided in Q-136): the `reserved` fields of `mp_output_config`, `mp_device_info`, `mp_engine_stats`, `mp_analysis_frame`, `mp_renderer_config` and `mp_latency_sample` are a store for future single-byte flags, kept so that a yes/no flag changes no struct size or offset. The core writes zero into them; a caller passing a struct in must zero them; a reader ignores any value it does not know; giving a byte a meaning is a minor ABI bump (as 0.11 did with `mp_analysis_frame.discontinuities`). A field wider than a byte is appended instead. `mpcore.tests` proves the core's zeros (`test_abi.cpp`, `test_latency.cpp`).
