...
- [x] Layout matches user-interface.md proportions at 1600 px and collapses correctly at 700 px (`--shell-spike` measures the live tree: 960/640 px at 1600 with the 960 px controls bar under Now Playing, all three panels spanning the client at 700. Until T-182 this read 960/400/240, with the controls a third column that clipped the transport; Phil moved it to a bar below the visualizer and widened the sidebar to 60/40)
...
- [x] Parameters (bar count, smoothing, colour source) are exposed in Settings › Visualization — **retired from this story to E4-S9 (T-60), and E4-S9 has since built that screen**, so the criterion is met where it was sent. What this story owns is done and tested: both presets declare the three parameters the line names (`bars`/`points`, `smoothing`, `colour`) plus `gain` and, for the waveform, `thickness`; each is reachable through `mp_renderer_set_param` and `IVisualizationHost.SetParameter`; and each is checked to change the rendered picture, with the colour source checked to change colour without changing which pixels are lit. `smoothing` is spatial rather than temporal because a preset has no state between frames — see visualization-engine.md. One gap for E4-S9 to know about: `mp_preset_info` carries `id` and `name` only, so a settings page cannot discover that `bars` runs 8–128 without being told out of band. Adding parameter metadata to the ABI is a minor and is not in this story

### E4-S5 · Built-in presets: Radial Spectrum, Ambient Glow · **M** · `render` `native`
Depends on: E3-S7, E4-S3, E4-S4. `presets/radial-spectrum` and `presets/ambient-glow`, written against E4-S3's constant-buffer contract and tested with E4-S4's golden-image harness — which needed no change to take them, which was the point of building it as one. What the two presets are and what their parameters mean is in [visualization-engine.md](visualization-engine.md), "The presets that ship".
- [x] Golden-image tests pass for a fixed `AnalysisFrame` (`mpcore.tests [golden]` renders each from the same fixed `mp_analysis_frame` E4-S4 built and compares the whole 640×360 target against a checked-in PNG; both match at max channel delta 0, 0 of 230400 pixels differing, in Debug, Release and ASan. Negative proof by perturbing one constant at a time: Radial Spectrum's outer edge 0.94 → 0.93 gives max channel delta 145 over 28006 pixels and one ramp stop's green −0.04 gives 10; Ambient Glow's brightness ceiling −1% gives a mean of 0.2282 over 146443 pixels and a 2% lobe-centre shift 0.1449 over 88294. **Ambient Glow needed a tighter mean bound to have a failure mode at all** — its picture lives in the bottom eighth of the byte range, so both perturbations passed the tolerances the other three use; that is the same class of finding as E4-S4's 320×180 goldens and was found the same way)
- [ ] Both render at 60 fps at 1080p on the reference iGPU — **not measured here; the reference machine's criteria collect in T-90.** What was measured, 1920×1080 headless over a 2 s window on the dev machine (i7-9700K, RTX 4080 SUPER, Windows 10 Pro 19045): Release on the RTX 4080 SUPER, Radial Spectrum 21 077 fps and Ambient Glow 21 888 fps; Release on WARP — software, so a CPU figure that owes nothing to the GPU in this box — Radial Spectrum 2287 fps and Ambient Glow 150 fps. `mpcore.tests` gates the WARP floor at 30 fps per preset, the same floor E4-S3's AC-119 holds, and prints the hardware figure without asserting it. Ambient Glow is the slowest of the four because it is one screen-covering quad and its cost is per-pixel
- [x] Parameters exposed in Settings › Visualization — **retired to E4-S9 (T-60), which has since built that screen**, on the same reading as E4-S4's. E4-S9 needed the one extra thing this story flagged and T-142 supplied it: the `art_*` parameters carry a `hidden` flag in the manifest and on the ABI, so the page offers Ambient Glow's five real parameters and none of its three packed integers. Both presets declare the parameters and each is reachable through `mp_renderer_set_param` and `IVisualizationHost.SetParameter` and checked to change the rendered picture. E4-S9 has one more thing to know than E4-S4 left it: Ambient Glow's three `art_*` parameters are set by code and not by a person, so a settings page that listed every declared parameter would offer a user three raw packed integers to type
- [x] Ambient Glow uses palette colours from album art when available (E3-S7's `ArtPalette` reaches the preset as `art_primary`/`art_secondary`/`art_accent` through the existing `mp_renderer_set_param`, one sRGB colour packed per float as `r*65536 + g*256 + b`, with −1 — the default — meaning "no art". `AmbientGlowPalette` in `Tunqio.Core` is the managed half and `mpcore.tests [art]` the rendered half: a (200,30,40) palette makes the field read red where the preset's own ramp reads blue, each of the three colours changes the picture, and one step out of 16 777 216 packed values is visible. "When available" is tested and not assumed — no art, a palette too dark to glow, and one dark colour in an otherwise usable palette all still draw. Deliberately **not** `mp_renderer_set_theme`, which stays E4-S6's stub: that is a renderer-wide theme needing a field in `b0`, which is a schema bump, and this is one preset's colour source, which the contract already expresses. What is still missing is the caller — nothing in the shell attaches the visualizer to Now Playing yet)
...
As built: the controller is `render/quality.h`, a pure object taking a clock reading and a frame cost, so its behaviour over time is measured against a synthetic series rather than waited for on a machine somebody else is also using. `mp_renderer_set_quality` is implemented — the last of E0-S5's stubs — and `mp_render_stats` grew a seven-field tail (ABI 0.16, appended, which T-140 made a minor). **Render scale is the only lever, and T-148 is why**: of the four shipped presets only `ambient-glow` can exhaust a rasteriser, and it is the only one whose cost is per-pixel, so a lever measured in pixels is the only one that helps the preset that needs helping. Update rate was refused on its own merits — presenting less often does not make a frame cheaper, and a tier that halves the frame rate reads as a tier failing its budget to any controller that measures the interval. **The 2 s hysteresis this line asked for is not what shipped, because it does not work**: a cooldown bounds how fast a controller oscillates and not whether it does. See [visualization-engine.md](visualization-engine.md), "Adaptive quality (as built)".
- [x] Forcing a slow GPU (WARP) drops quality to Low within 4 s and shows it in the diagnostics overlay (**1.74 s**, end to end on WARP with `ambient-glow` at 1080p, every frame time real. The budget is set from what the machine measures at each tier rather than from 16.667 ms, because whether WARP misses 60 fps depends on who else is using the CPU — 150 fps idle, 51.9 under a concurrent build, 21.4 on a shared runner — and a test that needs it to miss is the test T-150 deleted. Measured on the way: 6.99 ms at High, 4.99 at Medium, 2.10 at Low, from a D3D11 timestamp pair, which is also the proof that the lever does what the preset's shape says. The overlay half is `Diagnostics.Renderer`, tested as a pure function: `low (auto) · 427×360 at 0.5× · 2 changes`)
- [x] Restoring headroom returns to High within 10 s without oscillating (**4.77 s** end to end, with the surface cut to a sixteenth of the pixels and the budget untouched, in exactly two changes, held for three seconds more. "Without oscillating" is a **measured property and not an assertion**: the raise rule is a prediction — `smoothed × gain(tier→tier+1) ≤ budget × 0.8`, where the gain starts at the analytic per-pixel bound and is replaced by what the controller measured crossing that boundary — and a raise undone inside ten seconds doubles the wait before the next one, which makes the change count logarithmic in the window rather than linear. **Proved able to fail**: on a trap series the shipped controller changes tier once in two minutes, the escalation alone 11 times (exactly the derived bound), and the sketch in docs/performance-optimization.md 65 times; on 300 s no predictor can be right about, 8 against 75)
...
- [x] Switching presets takes < 200 ms with no black frame (**42.2 ms p95** against the 200 ms budget — `Tunqio.Benchmarks.PresetSwitchBenchmarks --gate`, cycling the four shipped presets headless on WARP so the figure owes nothing to the GPU in this box: median 39.6 ms, max 43.2 ms over n = 37, and nearly all of it is `D3DCompile`. A BenchmarkDotNet budget rather than a stopwatch in xunit, because two single-sample xunit budgets have already been retired on this project. "No black frame" is the half a number cannot express and is `mpcore.tests [render][preset][switch]`: a pixel read back across six switches between two presets that each cover the field in a colour of their own — 20 frames captured, 14 green, 6 blue, **0 blank**, and a frame that was neither would be a failure rather than a colour nobody noticed)
- [x] User presets dropped into `%LocalAppData%\Tunqio\presets\` appear after a refresh (the core now scans a **second** root the shell names, in addition to `presets/` beside `mpcore.dll`; the shipped root wins any id clash so a file cannot shadow `spectrum-bars`. "Refresh" is deliberately an explicit button and not a `FileSystemWatcher` — a manifest is written in pieces and a watcher would try to compile half of one — and a rescan never changes what is drawing, even one that deletes the active preset's own directory. Proved end to end by `tools/check-visualization-settings.ps1`, which writes a preset into the real user directory while the app is running, checks it is *not* in the list, presses Refresh, finds it, and then finds that its four controls — a stepped count, a `px` width, three named modes and no `art_*` — were all built from a file that was three seconds old)
...
- As built (T-78): `JumpListController` over `IJumpList` builds a Recent tracks group (up to ten, newest first, from Library › Recently played's query) and a Pinned playlists group, refreshes at start-up, when a finished listen is recorded (`PlaybackSession.PlayRecorded`) and when a playlist changes, and coalesces a burst into one write; `WinRtJumpList` writes it through `Windows.UI.StartScreen.JumpList`. Playlist detail has a Pin to jump list toggle over the new `IPlaylistRepository.SetPinnedAsync`. Items launch `tunqio://track?id=` and `tunqio://playlist?id=`, which `CommandRouter` plays in the running instance or a new one; a track or playlist that no longer exists is refused with one log line. **Proven unpackaged:** all of that in unit tests, and `tools/check-jump-list.ps1` on a scratch profile (the pin flag, an item's arguments played by a cold start and by the running instance, missing ids refused). **Waits for E8-S1 (T-80, Q-119 skip-until-release):** the API needs package identity, so the unpackaged build skips the jump list with one log line; the real taskbar list, its look and real clicks are checked on the installed package. See windows-integration.md, "As built (E7-S5)".
