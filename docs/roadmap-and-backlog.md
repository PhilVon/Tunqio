...
As built: the controller is `render/quality.h`, a pure object taking a clock reading and a frame cost, so its behaviour over time is measured against a synthetic series rather than waited for on a machine somebody else is also using. `mp_renderer_set_quality` is implemented — the last of E0-S5's stubs — and `mp_render_stats` grew a seven-field tail (ABI 0.14, appended, which T-140 made a minor). **Render scale is the only lever, and T-148 is why**: of the four shipped presets only `ambient-glow` can exhaust a rasteriser, and it is the only one whose cost is per-pixel, so a lever measured in pixels is the only one that helps the preset that needs helping. Update rate was refused on its own merits — presenting less often does not make a frame cheaper, and a tier that halves the frame rate reads as a tier failing its budget to any controller that measures the interval. **The 2 s hysteresis this line asked for is not what shipped, because it does not work**: a cooldown bounds how fast a controller oscillates and not whether it does. See [visualization-engine.md](visualization-engine.md), "Adaptive quality (as built)".
