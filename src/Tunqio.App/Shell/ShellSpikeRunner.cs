...
/// <summary>What the three panels actually measured at one client width, after the layout pass settled.</summary>
...
/// bound to it. At 1600 px the shares must be the documented 60 / 25 / 15; at 700 px the panels must have stacked,
/// which shows as all three spanning the full client width instead of dividing it.
/// </remarks>
internal sealed class ShellSpikeRunner
{
    ...
    private void Measure(int requested)
    {
        ...
        _logger.LogInformation(
            "shell spike at {Width} px: {Mode}, panels {NowPlaying:F0} / {Sidebar:F0} / {Controls:F0} px ({Note})",
    ...
    private void Finish()
    {
        ...
        var report = new
        {
            ...
            Verdict = pass
                ? "PASS: 60/25/15 at 1600 px, panels stacked at 700 px, and every theme switch applied in place without rebuilding the tree"
    ...
    /// <summary>
    /// Whether one measurement is what the width called for: the documented shares in a column shape, and three
    /// panels each spanning the client in a stacked one.
    /// </summary>
    internal static (bool Pass, string Note) Judge(ShellLayoutState expected, double clientWidth, double nowPlaying, double sidebar, double controls)
    {
        ...
        if (nowPlaying < ShellLayout.NowPlayingMinWidth - 1
            || sidebar < ShellLayout.SidebarMinWidth - 1
            || controls < ShellLayout.ControlsMinWidth - 1)
        {
            return (false, string.Create(
                CultureInfo.InvariantCulture,
                $"a panel is under its floor: {nowPlaying:F0} / {sidebar:F0} / {controls:F0} px against floors of " +
                $"{ShellLayout.NowPlayingMinWidth:F0} / {ShellLayout.SidebarMinWidth:F0} / {ShellLayout.ControlsMinWidth:F0}"));
        }

        double gotNowPlaying = nowPlaying / total;
        double gotSidebar = sidebar / total;
        double gotControls = controls / total;
        if (ShellLayout.FloorsBind(clientWidth))
        {
            ...
            return (true, string.Create(
                CultureInfo.InvariantCulture,
                $"floors bind at this width, so the shares are {gotNowPlaying:P0} / {gotSidebar:P0} / {gotControls:P0} rather than the documented ones"));
        }

        (double wantNowPlaying, double wantSidebar, double wantControls) = expected.Fractions;
        bool ok = Math.Abs(gotNowPlaying - wantNowPlaying) <= Tolerance
            && Math.Abs(gotSidebar - wantSidebar) <= Tolerance
            && Math.Abs(gotControls - wantControls) <= Tolerance;
        return (ok, string.Create(
            CultureInfo.InvariantCulture,
            $"wanted {wantNowPlaying:P0} / {wantSidebar:P0} / {wantControls:P0}, measured {gotNowPlaying:P0} / {gotSidebar:P0} / {gotControls:P0}"));
    }
